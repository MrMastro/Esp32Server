// HttpUtils.js
const HttpUtils = (() => {
    const cordova = {platformId:'android'};
    // Funzione principale per inviare una richiesta POST
    const postCustom = async (host: any, url: any, queryParam: any, postData: any) => {
    ...
    const getCustom = async (host: any, url: any, queryParam: any, postData: any) => {
        url = host + url; // Concatenare l'host con l'URL fornito
    ...
    const postWithAndroid = async (url: string, options: { method?: string; data: any; param: any; }) => {
    ...
    // Funzione per gestire la richiesta POST su Android
    const getWithAndroid = async (url: string, options: { method?: string; data?: any; param: any; }) => {
        let result;
        if (options.param && JSON.stringify(options.param) !== '{}') {
            url += `?${options.param}`; // Aggiungere i parametri all'URL
        }
    ...
    const postWithBrowser = async (url: string | URL, options: { param: string | number | boolean; data: any; }) => {
        if (options.param) {
    ...
    const postWithBrowserCors = async (url: string | URL, options: { param: string | number | boolean; data: any; }) => {
        if (options.param) {
            url += `?${encodeURIComponent(options.param)}`; // Aggiungere i parametri all'URL
        }
