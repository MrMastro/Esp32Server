// src/services/Esp32ConnectionService.js

import HttpUtils from '../utils/HttpUtils';
import IpV4StringUtils from '../utils/IpV4StringUtils';
...
export default class Esp32ConnectionService {
    private referenceHost: string;
    private context: any; // Adjust the type based on your actual context
    private linkedDeviceSearch: Array<any>; // Adjust the type based on your actual data structure
    ...
    async searchLinkedDevice(ipV4: any, addIpToListKnown: boolean, callBackWhenDeviceFound = null) {
        this.linkedDeviceSearch = [];
    ...
    setSingleDeviceOffline(espConnection: { infoConnection: { macAdress: any; }; }) {
        let updateList = this.localStorageService.getEsp32InfoDeviceMem();
        updateList.forEach((esp32: { infoConnection: { macAdress: any; }; connectionState: string; active: boolean; }, i: any) => {
    ...
    async createFakeEsp32(name: string, ip: string, mac: string) {
        let json = {
            "deviceName": name,
            "ip": ip,
            "macAdress": mac
